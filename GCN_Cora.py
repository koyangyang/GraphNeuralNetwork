...
def train_step(
    ...
    acc = preds.eq(y).sum().item() / mask.sum().item()
    loss.backward()
    optimizer.step()
...
@torch.no_grad()
def eval_step(
    model: torch.nn.Module, data: Data, loss_fn: LossFn, stage: Stage
) -> Tuple[float, float]:
    model.eval()
...
def train(
    ...
    for epoch in range(max_epochs):
        ...
        if epoch > early_stopping and val_loss > np.mean(
            history["val_loss"][-(early_stopping + 1) : -1]
        ):
            if verbose:
                print("\nEarly stopping...")
...
def plot_history(
    history: HistoryDict, title: str, font_size: Optional[int] = 14
) -> None:
...
history = train(
    model, data, optimizer, max_epochs=MAX_EPOCHS, early_stopping=EARLY_STOPPING
)
